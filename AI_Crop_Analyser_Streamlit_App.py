...
st.title("🌱 AI Crop Analyser — Streamlit App")
...
def recommend_renewable_action(row):
    ...
    if row['soil_moisture'] < 25:
        if row['sunlight_lux'] > 30000:
            recs.append("Schedule solar-powered irrigation during peak sun hours (10:00–14:00).")
        else:
...
if mode == "Sensor-data (train & predict)":
    st.header("Sensor data — train a model to predict crop health")
    ...
    if do_train:
        ...
        st.success(f"Training finished — test accuracy: {acc:.3f}")
...
##########################
    ...
    st.header("Image Inference — Upload an image to get predictions")
    if not TF_AVAILABLE:
...
##########################
    ...
    st.header('About — AI Crop Analyser')
    st.markdown(
        '''
...
# Footer
st.markdown('---')
st.caption('AI Crop Analyser — starter app. Developed by Archit + ChatGPT (assistant).')